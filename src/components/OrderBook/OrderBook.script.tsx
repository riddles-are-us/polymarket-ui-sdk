...
const updateOrders = (orders: OrderItem[], basePrice: number, isAsk: boolean, fixedCount: number, otherSideLowestPrice?: number): OrderItem[] => {
  ...
  if (updatedOrders.length < fixedCount) {
    ...
    for (let i = 0; i < missingCount; i++) {
      ...
      const lastOrder = isAsk 
        ? updatedOrders.length > 0 ? updatedOrders[updatedOrders.length - 1] : null
        : updatedOrders.length > 0 ? updatedOrders[0] : null;
...
export const useOrderBook = () => {
  ...
  const [asks, setAsks] = useState(() => generateMockOrders(basePrice, fixedOrderCount, true).sort((a, b) => a.price - b.price));
  const [bids, setBids] = useState(() => generateMockOrders(basePrice, fixedOrderCount, false).sort((a, b) => b.price - a.price));
  const timerRef = useRef<NodeJS.Timeout | null>(null);
  
  // Calculate price direction
  const priceDirection = lastPrice > previousPrice 
    ? "up" 
  ...
  useEffect(() => {
    const updateOrderBook = () => {
      ...
      const currentLowestAsk = asks.length ? asks[0].price : parseFloat((basePrice + 0.5).toFixed(2));
      const currentHighestBid = bids.length ? bids[0].price : parseFloat((basePrice - 0.5).toFixed(2));
      
      // Ensure reasonable spread between buy/sell prices
      const minSpread = 0.02; // Minimum price difference
  ...
  const spread = asks.length && bids.length 
    ? Number((asks[0].price - bids[0].price).toFixed(2))
    : 0;
